...
import gov.samhsa.bhits.pcm.service.dto.SpecificMedicalInfoDto;
...
public class ConsentHelper {
	...
	protected boolean isDatesOverlap(Date selStartDate, Date selEndDate,
		...
		final Instant selStartInstant = selStartDate.toInstant();
		final Instant selEndInstant = selEndDate.toInstant();
		final Instant actStartInstant = actStartDate.toInstant();
		final Instant actEndInstant = actEndDate.toInstant();
		Assert.isTrue(selStartInstant.compareTo(selEndInstant) <0, "SelectedStartDate is greater than SelectedEndDate");
		Assert.isTrue(actStartInstant.compareTo(actEndInstant) <0, "ActualStartDate is greater than ActualEndDate");
		// (StartA <= EndB) and (EndA >= StartB)
		return selStartInstant.compareTo(actEndInstant) <=0 && selEndInstant.compareTo(actStartInstant) >=0;
	}
