package gov.samhsa.pcm.service.audit.pg;
...
import gov.samhsa.pcm.domain.audit.ModifiedEntityTypeEntity;
import gov.samhsa.pcm.domain.audit.ModifiedEntityTypeEntityRepository;
import gov.samhsa.pcm.domain.audit.RevisionInfoEntity;
...
public class AuditServiceImplTest {
	...
	public void testFindHistoryDetail() {
		...
		when(patientRepository.findByUsername(anyString())).thenReturn(patient);
		when(staffRepository.findByUsername(anyString())).thenReturn(null);
	...
	public void testmakeHistoryDto_return_correct_class() {
		...
		String className = object.getClass().getName();
		assertEquals("gov.samhsa.pcm.service.dto.HistoryDto",
				className);
	}
	...
	public void testFindRevClassName_when_modifiedEntity_size_is_one() {
		...
		when(modifiedEntityTypeEntitys.get(0).getEntityClassName()).thenReturn(
				"gov.samhsa.pcm.domain.patient.Patient");
	...
	public void testFindRevClassName_when_modifiedEntity_size_is_two_Second() {
		...
		when(modifiedEntityTypeEntitys.get(0).getRevisionType()).thenReturn(
				(byte) 0);
		when(modifiedEntityTypeEntitys.get(0).getEntityClassName()).thenReturn(
				"gov.samhsa.pcm.domain.provider.IndividualProvider");
