...
$(function(){
	...
	$("#provider_search_modal #resultList").on("click", "button.addProviderButton", function(evt){
		...
		if($(clickedButton).hasClass("addIndividualProviderButton")){
			var entryId = null;
			var serializedQueryResult = null;
			
			//Disable button to prevent multiple clicks
			$(clickedButton).prop("disabled", true);
			
			entryId = clickedButton.attr("id").substr(27,clickedButton.attr("id").length-27);
			serializedQueryResult = JSON.stringify(result4ajaxJSON["providers"][entryId]);
				...
				default:
					...
					$.ajax({
						  ...
						  success:function(response) {
							  ...
							  $('div#disclose-list-container h2:contains("Organizations:")').before("<div>" +
							  			"<label for='to" + result4ajaxJSON["providers"][entryId].npi + "'>" +
							  					"<input class='isMadeToList indv-prov' id='to" + result4ajaxJSON["providers"][entryId].npi + "' " +
							  						...
							  						"type='radio' name='providersDisclosureIsMadeTo' style='float: none;' />" +
							  ...
							  $('div#authorize-list-container').append("<div>" +
									  	"<label for='from" + result4ajaxJSON["providers"][entryId].npi + "'>" +
					  						"<input class='toDiscloseList indv-prov' id='from" + result4ajaxJSON["providers"][entryId].npi + "' " +
					  							"value='" + result4ajaxJSON["providers"][entryId].npi + "' " +
					  							"type='radio' name='providersPermittedToDisclose' style='float: none;' />" +
			...
			entryId = $(this).attr("id").substr(31,$(this).attr("id").length-31);
			serializedQueryResult = JSON.stringify(result4ajaxJSON["providers"][entryId]);
							  ...
							  $('div#disclose-list-container').append("<div>" +
							  			"<label for='to" + result4ajaxJSON["providers"][entryId].npi + "'>" +
							  					"<input class='isMadeToList org-prov' id='to" + result4ajaxJSON["providers"][entryId].npi + "' " +
							  						"value='" + result4ajaxJSON["providers"][entryId].npi + "' " +
							  						"type='radio' name='organizationalProvidersDisclosureIsMadeTo' style='float: none;' />" +
							  ...
							  $('div#authorize-list-container').append("<div>" +
							  			"<label for='from" + result4ajaxJSON["providers"][entryId].npi + "'>" +
							  					"<input class='toDiscloseList org-prov' id='from" + result4ajaxJSON["providers"][entryId].npi + "' " +
							  						"value='" + result4ajaxJSON["providers"][entryId].npi + "' " +
							  						"type='radio' name='organizationalProvidersPermittedToDisclose' style='float: none;' />" +
